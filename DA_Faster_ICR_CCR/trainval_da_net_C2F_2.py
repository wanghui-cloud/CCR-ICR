...
def parse_args():
    ...
    # 传入已训练过的模型 resume trained model
    parser.add_argument("--r", dest="resume", help="resume check  point or not", action='store_true')
    parser.add_argument("--model_name", dest="model_name", default="C2F_20.pth",
                        help="resume from which model", type=str,)

    # 使用的数据集   C2F  S2C  V2C  K2C  C2BDD
    parser.add_argument("--dataset", dest="dataset", default="C2F", help="train dataset", type=str,)
    # 选用的模型和存放的位置
    parser.add_argument("--net", dest="net", default="vgg16", help="vgg16, res101", type=str)
    parser.add_argument("--pretrained_path", dest="pretrained_path", default="",
                        help="vgg16, res101", type=str,)
    # 保存模型的迭代次数
    parser.add_argument("--checkpoint_interval", dest="checkpoint_interval", default=1,
                        help="number of iterations to save checkpoint",type=int,)
    # 模型的保存位置
    parser.add_argument("--save_dir", dest="save_dir", default="./data/experiments/DA_Faster_ICR_CCR/C2F/model",
                        help="directory to save models", type=str,)
    # 训练epoch配置
    parser.add_argument("--max_epochs", dest="max_epochs", default=50, help="max epoch for train", type=int, )
    parser.add_argument("--start_epoch", dest="start_epoch", default=1, help="starting epoch", type=int)
    # tenrosbord显示
    parser.add_argument('--use_tfb', dest='use_tfboard', default='True', help='whether use tensorboard', action='store_true')
    # 加载数据时使用的多线程加载的进程数
    parser.add_argument("--nw", dest="num_workers", default=8, help="number of worker to load data", type=int,)
    ...
    parser.add_argument("--bs", dest="batch_size", default=1, help="batch_size", type=int)
    # 是否执行类无关的bbox回归
    parser.add_argument("--cag", dest="class_agnostic", action="store_true",
                        help="whether perform class_agnostic bbox regression",)
    # config optimization
    parser.add_argument("--max_iter", dest="max_iter", default=10000, help="max iteration for train", type=int,)
    parser.add_argument("--o", dest="optimizer", default="sgd", help="training optimizer", type=str)
    # 学习率设置
    parser.add_argument("--lr", dest="lr", default=0.001, help="starting learning rate", type=float) # 初始学习率
    parser.add_argument("--lr_decay_step", dest="lr_decay_step", default=5,
                        help="step to do learning rate decay, unit is iter", type=int,)
    parser.add_argument("--lr_decay_gamma", dest="lr_decay_gamma", default=0.1,
                        help="learning rate decay ratio", type=float,)    # 学习率下降率
    # 实例级权重？？
    parser.add_argument("--instance_weight_value", dest="instance_weight_value", default=1.0,
                        help="instance_weight_value", type=float,)
    # DA损失函数参数
    parser.add_argument("--lamda", dest="lamda", default=0.1, help="DA loss param", type=float)

    # 设置显示配置, 代中多少个batch显示
    parser.add_argument("--disp_interval", dest="disp_interval", default=1000,
...
if __name__ == "__main__":
    ...
    if args.dataset == "C2F":
        ...
        args.set_cfgs = ["ANCHOR_SCALES", "[8,16,32]",
                         ...
                         "MAX_NUM_GT_BOXES", "30", ]
        # args.set_cfgs = ["ANCHOR_SCALES", "[32, 64, 128, 256, 512]",
        #                  "ANCHOR_RATIOS", "[0.5 , 1, 2]",
        #                  "MAX_NUM_GT_BOXES", "30", ]
    # Synthetic-to-Real:sim10k——>cityscapes_car
        ...
        # 使用时，需要把元组换成列表
        args.set_cfgs = ["ANCHOR_RATIOS", "[0.5,1,2]", "TRAIN.SCALES", "[800,]",
                         "TRAIN.MAX_SIZE", "1600", "TEST.SCALES", "[800,]",
                         "TEST.MAX_SIZE", "1600", ]
