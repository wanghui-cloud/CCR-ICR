...
def parse_args():
    ...
    parser.add_argument("--save_dir", dest="save_dir",
                        default="./data/experiments/DA_Faster_ICR_CCR/C2F_fix1/model",
    ...
    parser.add_argument('--use_tfb', dest='use_tfboard',
                        help='whether use tensorboard', action='store_true')
    # 加载数据时使用的多线程加载的进程数
...
if __name__ == "__main__":
    ...
    # 训练集加载及处理
    cfg.TRAIN.USE_FLIPPED = False  # 数据集增强方式
    ...
    for epoch in range(args.start_epoch, args.max_epochs + 1):
        ...
        for step in range(iters_per_epoch):  # 分为batch训练
            ...
            # '''###############         验证数据集      #################'''
            # if args.vis:
            #     # im = np.copy(im_data.cpu().squeeze(0))
            #     i=s_imdb.image_index
            #     im = cv2.imread(s_imdb.image_path_at(i))
            #     temp_bbox = np.copy(gt_boxes.cpu().squeeze(0))   # 30,5
            #     for i in range(num_boxes):
            #         bbox = tuple(int(np.round(x)) for x in temp_bbox[i, :4])
            #         j = temp_bbox[i, -1]
            #
            #         cv2.rectangle(im, bbox[0:2], bbox[2:4], (0, 204, 0), 2)
            #         cv2.putText(im, "%s: %.3f" % (s_imdb.classes[int(j)], 1), (bbox[0], bbox[1] + 15),
            #                     cv2.FONT_HERSHEY_PLAIN, 1.0, (0, 0, 255), thickness=8, )
            #     cv2.imwrite("vis_source.png", im)
            #
            #
            #     # tgt_im = np.copy(tgt_im_data.cpu().squeeze(0))
            #     tgt_im = cv2.imread(t_imdb.image_path_at(t_imdb.image_index))
            #     temp_tgt_bbox =  np.copy(tgt_gt_boxes.cpu().squeeze(0))
            #     for i in range(tgt_num_boxes):
            #         j = temp_tgt_bbox[i, -1]
            #         bbox = tuple(int(np.round(x)) for x in temp_tgt_bbox[:, :4])
            #
            #         cv2.rectangle(tgt_im, bbox[0:2], bbox[2:4], (0, 204, 0), 2)
            #         cv2.putText(tgt_im, "%s: %.3f" % (t_imdb.classes[int(j)], 1), (bbox[0], bbox[1] + 15),
            #                     cv2.FONT_HERSHEY_PLAIN, 1.0, (0, 0, 255), thickness=8, )
            #     cv2.imwrite("vis_target.png", tgt_im)
