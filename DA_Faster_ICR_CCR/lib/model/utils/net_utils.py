...
# 输入： im：图片信息、class_name：类别名称、dets:[i,5]
...
def _affine_grid_gen(rois, input_size, grid_size):
    ...
    zero = Variable(rois.data.new(rois.size(0), 1).zero_())
    theta = torch.cat([(x2 - x1) / (width - 1), zero,
                       (x1 + x2 - width + 1) / (width - 1), zero,
                       (y2 - y1) / (height - 1),
                       (y1 + y2 - height + 1) / (height - 1),],1,).view(-1, 2, 3)
...
def _affine_theta(rois, input_size):
    ...
    theta = torch.cat([ (y2 - y1) / (height - 1), zero,
                        (y1 + y2 - height + 1) / (height - 1), zero,
                        (x2 - x1) / (width - 1),
