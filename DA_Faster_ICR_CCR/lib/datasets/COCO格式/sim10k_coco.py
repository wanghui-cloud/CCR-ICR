...
class sim10k(imdb):
    def __init__(self, image_set, year):
        ...
        self._data_path = osp.join(cfg.DATA_DIR, "datasets/sim10k")
        # self._devkit_path = (cfg_d.CLIPART)
        # self._data_path = (self._devkit_path)
        # load COCO API, classes, class <-> id mappings
        # 原始COCO的官方API提供的一个解析对象
        self._COCO = COCO(self._get_ann_file())
        cats = self._COCO.loadCats(self._COCO.getCatIds())
        # 加载了COCO的类别
        self._classes = tuple(["__background__"] + [c["name"] for c in cats])
        # 对COCO的类被与类别ID之间建立了一个映射
        self._class_to_ind = dict(list(zip(self.classes, list(range(self.num_classes)))))
        # 对每张图像的类别与类别ID之间建立了映射
        ...
        self._data_name = (self._view_map[coco_name] if coco_name in self._view_map
                           else coco_name)
    ...
    """
        ...
        return { "width": width, "height": height, "boxes": boxes,
                 "gt_classes": gt_classes, "gt_overlaps": overlaps,
    ...
    def append_flipped_images(self):
        ...
        for i in range(num_images):
            ...
            entry = { "width": widths[i], "height": self.roidb[i]["height"],
                      "boxes": boxes, "gt_classes": self.roidb[i]["gt_classes"],
    ...
    def _coco_results_one_category(self, boxes, cat_id):
        ...
        for im_ind, index in enumerate(self.image_index):
            ...
            ws = dets[:, 2] - xs + 1
            hs = dets[:, 3] - ys + 1
            results.extend([{"image_id": index, "category_id": cat_id,
    ...
    def evaluate_detections(self, all_boxes, output_dir):
        res_file = osp.join(output_dir, ("detections_" +
                                         self._image_set + self._year + "_results"))
