...
# 输入就是xml格式的标注文件，通过obj变量获得x1，y1，x2，y2，即标注信息，以及cls类别信息
# 并标注overlaps等于1。另外seg_areas实际上就是标注框的面积。
...
class pascal_voc(imdb):
    ...
    def _load_selective_search_roidb(self, gt_roidb):
        filename = os.path.abspath(os.path.join(
            cfg.DATA_DIR,"selective_search_data", self.name + ".mat") )
    ...
    def _load_pascal_annotation(self, index):
        ...
        return {"boxes": boxes, "gt_classes": gt_classes,
                ...
                "flipped": False, "seg_areas": seg_areas, "seg_map": seg_map,}
